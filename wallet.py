...
class Wallet:
    ...
    def sign_transaction(self, sender, recipient, amount):
        signer = PK.new(RSA.importKey(binascii.unhexlify(self.private_key)))
        h = SHA256.new((str(sender) + str(recipient) +
                        str(amount)).encode('utf8'))
        signiture = signer.sign(h)
        return binascii.hexlify(signiture).decode('ascii')
    ...
    def verify_transaction(transaction):
        ...
        return verifier.verify(h, binascii.unhexlify(transaction.signiture))
